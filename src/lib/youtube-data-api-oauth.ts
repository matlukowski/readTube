...
import { getServerYouTubeToken } from './google-oauth-helper';
...
export async function getAvailableCaptionsOAuth(
  ...
  console.log(`🔍 Fetching available captions for ${videoId} with Google OAuth2...`);
  
  try {
    const accessToken = await getServerYouTubeToken(googleId);
...
export async function downloadCaptionOAuth(
  ...
  console.log(`📥 Downloading caption ${captionId} in ${format} format with Google OAuth2...`);
...
export async function getVideoMetadataOAuth(
  ...
  console.log(`📊 Fetching video metadata for ${videoId} with Google OAuth2...`);
...
export async function fetchYouTubeTranscriptOAuth(
  ...
  console.log(`🎯 Fetching YouTube transcript using Google OAuth2 for user ${googleId}...`);
  
  try {
    // Step 1: Get video metadata
    const metadata = await getVideoMetadataOAuth(videoId, googleId);
    console.log(`📺 Video: "${metadata.title}" by ${metadata.channel}`);
    
    // Step 2: Get available captions
    const captions = await getAvailableCaptionsOAuth(videoId, googleId);
    ...
    const captionContent = await downloadCaptionOAuth(selectedCaption.id, googleId, 'srt');
    ...
    console.log(`✅ Google OAuth2 transcript ready: ${fullTranscript.length} characters, ${entries.length} segments`);
    ...
    return {
      ...
      metadata,
      source: `youtube-google-oauth-${selectedCaption.snippet.language}${selectedCaption.snippet.isAutoSynced ? '-auto' : ''}`
    };
    
  } catch (error) {
    console.error('❌ YouTube Google OAuth2 transcript fetch failed:', error);
    throw error;
  }
}
...
export async function getYouTubeTranscriptOAuth(
  ...
  const result = await fetchYouTubeTranscriptOAuth(videoId, googleId, language);
